...
│   ├── 📄 schema.ts                # sessions & conversations tables + generated search_text column + GIN index
│   └── 📄 seed.ts                  # idempotent local-dev seed
└── 📂 lib
    ├── 📄 server.ts                # Cookie session, origin guard, session-mint throttle, bounded body reader
    ├── 📄 client-key.ts            # Pure per-network key derivation (cf-connecting-ip → rightmost XFF → direct)
...
- Live provider access requires a valid key and model entitlement. The missing-key path is tested; live inference was verified end to end during audit pass 6 (2026-09-10, with a then-valid key), re-checked on 2026-09-11 (then rejected), and verified working again on 2026-09-13 — the deployed key now streams, persists, and cleans up a nonce-marked round trip (12/12 live E2E).
...
| `/api/conversations` | GET | Session cookie (sets one) | List up to 100 conversations + `configured` flag; optional `?q=` filters server-side by title and message content through the trigram-indexed `search_text` generated column (case-insensitive, owner-isolated, whitespace-collapsed) — rate-limited to 10 searches / 10 s / session (429 beyond; plain listing unlimited); new-session minting is bounded to 60 / 10 s / network |
...
Limits: 100 conversations per workspace, 60 persisted messages per conversation, 16,000 characters per submitted prompt, 3 MB request body, ~16 MB existing history (was 8 MB), and 1,200,000 response characters (was 600k; includes thinking + answer); server-side search is bounded to 10 queries / 10 s / session (plain listing unlimited) and new-session creation to 60 / 10 s / network. One generation per session is enforced by an atomic database lease (195s; 615s for `max_tokens > 16,384`) with a 3-second spacing; provider requests time out after 175s (590s for large outputs) and the route allows up to 600s (`maxDuration`), keeping the SSE connection alive with 15-second comment frames while the provider is silent so proxy idle timeouts cannot cut it first. No automatic inference retries are used, avoiding duplicate billing and ambiguous persisted turns; users can retry explicitly.
...
- The CSP baseline sets `default-src 'self'` with explicit resource directives (script/style allow same-origin plus the inline bootstrap the App Router prerenders and the Cloudflare Web Analytics beacon origins — the documented ingress injects that beacon zone-wide; images allow `data:` for composer previews and the lightbox; connect-src allows the beacon's reporting endpoint) plus framing, base-URL, plugin, form-action, and connect-src restrictions. Because prerendered HTML carries no script nonce, `script-src` keeps `'unsafe-inline'`; a deployment-specific nonce-based script policy (which forces dynamic rendering) remains the final hardening step.

## Testing

```bash
npm test                                   # unit: schemas, SSE parser (incl. comment frames), keep-alive helper, history grouping, load-failure copy (node:test)
...
Latest verification (2026-09-13, session 5): typecheck, lint, build, 62/62 unit tests (55 pass-9 baseline + 7 client-key tests), 38/38 Playwright tests locally (34 pass-9 baseline + the B1 index test, the lease-conflict 429 test, the session-mint throttle test, and the export-download test, including axe WCAG AA), and live-deployment E2E against `https://kimi-chat.jesspete.shop/`: **12/12 — the operator has redeployed with a valid provider key since pass 9. Both pass-9 failures are resolved live: (1) the deployed CSP now carries the Cloudflare Web Analytics origins (no beacon console errors), and (2) the provider round-trip works end to end — a nonce-marked send streams, renders, persists (verified through the read API with reasoning stripped), and cleans up its own conversation.** An additional exploratory browser pass (rename, export download, message-content search, image attach/remove, multi-turn, delete) found zero issues and zero console errors. Session 5 also closed the pass-9 remediation backlog's code items — indexed search (B1), lease-conflict coverage (I2), the app-level session-mint bound (M3), and the export-download test — each via TDD; see the change table below and the pass-10 audit section. Full evidence and the severity-ranked finding list live in [`docs/CODE_REVIEW_REPORT.md`](docs/CODE_REVIEW_REPORT.md) (passes 9–10).

| Change | Notes |
|--------|-------|
| Live deployment fully green (2026-09-13) | Verified 12/12 live E2E (the two pass-9 failures — CSP beacon block and provider stall — both resolved by the operator's redeploy): headers (hardened CSP + Cloudflare analytics origins, COOP/CORP, HSTS), Secure/Strict session cookie, exact-403 cross-origin writes, API contract, session isolation, WCAG AA, and a full provider round-trip (streamed, persisted, reasoning stripped, self-cleaning). An exploratory pass beyond the suite (rename, export, search-by-content, image attach/remove, multi-turn, delete) found zero issues; two short test conversations from a first exploratory attempt remain in the live DB under an orphaned session cookie until the retention prune runs |
| Indexed server-side search (2026-09-13) | Backlog B1 closed: `conversations.search_text` is a database-maintained STORED generated column (`title || E'\n' || messages_content_text(messages)` — an IMMUTABLE helper extracting message contents and excluding image payloads) served by the `conversations_search_idx` GIN `gin_trgm_ops` index (migration `drizzle/0001_lush_arachne.sql`, which also ensures `pg_trgm` and is self-contained for CI's service container). The `?q=` path now runs one indexed ILIKE instead of expanding every conversation's JSONB, closing the pass-9 M-1 amplification at the storage layer; search terms have whitespace runs collapsed server-side so newline-crafted terms cannot span the title/content join. The 10 / 10 s / session search limit stays as defense-in-depth (sub-three-character terms cannot use the trigram index); the E2E suite pins the column, the index, and unchanged API behavior. Deploy note: the migration backfills the column and builds the index under a brief ACCESS EXCLUSIVE lock — fine at the current data volume, schedule accordingly at scale |
| Lease is the uniform admission gate (2026-09-13) | The chat route's provider-key check moved after the lease claim (backlog I2): conflict behavior (429) is now reachable and covered by tests without a provider key — both the busy-window and the 3 s spacing refusals, plus recovery to the curated 503. An immediate retry (< 3 s) after a missing-key 503 now sees the 429 "sent too quickly" copy (the lease stamps `lastRequest`), and the key-check throw releases the lease |
| Session-mint throttle per network (2026-09-13) | Backlog M3's app-level portion: cookieless session-row creation (the cheapest unauthenticated write surface) is bounded to 60 new sessions / 10 s / network. The key prefers `cf-connecting-ip`, falls back to the rightmost `x-forwarded-for` value, and validates IP shapes (`src/lib/client-key.ts`, 7 unit tests) so spoofed first values cannot mint fresh buckets or retain oversized key strings; requests with a valid cookie are never throttled; direct/dev connections share one "direct" bucket. Ingress-level limits remain the documented public-service defense |
| Export coverage + audit hardening (2026-09-13) | The markdown export has an E2E test (filename `kimi-{id}.md`, `# title` / `## You` / `## Kimi` sections, hermetic via routed API fixtures). Pass-10 audit fixes: the mint-throttle flood test fires concurrently (window-safe on slow CI) with precise Set-Cookie-token cleanup, and the throttle module header documents both call sites |
| Search rate limit (2026-09-12) | The `?q=` search is bounded to 10 requests / 10 s / session (`src/lib/throttle.ts`, process-local sliding window with an entry-capped key map) after the pass-9 audit empirically verified resource amplification: at the documented caps, one search over image-heavy conversations costs 2.3–2.8 s of DB CPU and ~1.6 GB of TOAST reads. Plain listing stays unlimited; a throttled search returns a curated 429 and the client already degrades to local-title filtering behind its retryable notice. The indexed search path above landed in session 5 as the complete closure |
| Error responses are never cached (2026-09-12) | `errorResponse` now sets `Cache-Control: no-store` on every error body — the same contract the success paths already carried — and the malformed-input API test pins it |
| Dev database binds loopback only (2026-09-12) | `docker-compose.yml` publishes PostgreSQL on `127.0.0.1:5433` instead of all interfaces (the credential is repo-published; pass-9 L-1) |
| Keep-alive beat hardened (2026-09-12) | The comment-frame emitter wraps its enqueue in try/catch so an errored (non-canceled) stream teardown can never surface an uncaught timer exception (pass-9 L-2); heartbeat tests hardened against event-loop starvation (poll-until-deadline) |
| Cloudflare Analytics origins in CSP (2026-09-12) | The hardened CSP baseline now allows the Cloudflare Web Analytics beacon (`script-src … https://static.cloudflareinsights.com`, `connect-src … https://cloudflareinsights.com`). Verified live: zones with Web Analytics enabled inject the beacon zone-wide, and the previous policy blocked it — a console error on every page load and a failing live E2E check. Operators not using Web Analytics can disable it in the Cloudflare dashboard and drop the entries (header test pins both) |
| SSE keep-alive while the provider is silent (2026-09-12) | The chat route now emits `: keep-alive` SSE comment frames every 15 s from `meta` until the provider answers (and throughout the stream), so proxy idle timeouts (Cloudflare ~100 s, nginx `proxy_read_timeout` 60 s default) can no longer cut the browser connection before the route's own 175 s / 590 s provider timeout delivers its curated error — a verified live failure mode where a hung provider left the wire silent and the user saw only "The connection was interrupted" after ~100 s. Comment frames are ignored by the shared SSEParser; unit tests cover the helper and parser tolerance, and a stream-UI E2E test pins the browser-side contract |
| Hardened CSP baseline + COOP/CORP (2026-09-11) | The CSP now sets `default-src 'self'` with explicit `script-src 'self' 'unsafe-inline'` (prerendered App Router HTML ships nonce-less inline bootstrap scripts; dev adds `'unsafe-eval'` for React refresh), `style-src`, `img-src 'self' data:` (composer previews + lightbox), `font-src`, `connect-src 'self'`, and `form-action 'self'`; `Cross-Origin-Opener-Policy`/`Cross-Origin-Resource-Policy: same-origin` added. Closes the app-level portion of carried I3 (pass 8, TDD: header test red → green; full 32-test suite passes under the new policy). A nonce-based script policy remains the deploy-time hardening step; headers reach the live site on the next redeploy |
| Test locator contract aligned (2026-09-11) | Two tests used unscoped `getByRole("alert")` (text-filtered, so safe in practice) where the documented rule requires targeting `.error-banner`; both aligned with explanatory comments (pass 8) |
| Mutation-failure curated copy (2026-09-11) | A failed rename/delete (e.g. dropped connection during the write) shows "This action could not be completed. Check your network and try again." instead of raw transport text (`Failed to fetch`) — closing the same client-error boundary pass 6 established for send/open in the last remaining surface, the mutation toast (regression-tested) |
| Whitespace-safe auto titles (2026-09-11) | New-conversation titles derived from the first prompt now collapse whitespace runs (multi-line prompts no longer store newline-laden titles) and enforce a dual cap — 70 code points without splitting surrogate pairs AND ≤100 UTF-16 units so the rename schema always accepts them (`src/lib/title.ts`, 10 unit tests) |
| 409 delete-while-generating guard tested (2026-09-11) | Regression test proves DELETE returns 409 with the documented copy while a generation holds the lease, then succeeds once the lease expires (closes the coverage gap for the FOR UPDATE guard) |
| Live provider key verified dead (2026-09-11) | Fresh-clone live verification: the deployed `NVIDIA_API_KEY` is rejected by NVIDIA (401/403) while `configured: true` — every other live check (headers, Secure cookie, exact-403 cross-origin, API contract, session isolation, WCAG) passes. Key rotation is the operator action that restores the round-trip |
...
| `npm run db:migrate` / `db:seed` / `db:setup` → `PostgreSQL not reachable at DATABASE_URL` | PostgreSQL is down or `DATABASE_URL` points at the wrong host/port. Run `docker compose up -d` and `curl /api/health`; check `DATABASE_URL` matches `docker-compose.yml` (`127.0.0.1:5433:5432`, `chat_user/chat_secret`) |
| `npm run db:seed → {"inserted":0,"reason":"already seeded"}` | Not a failure — the DB already has sessions. `inserted:1` only on a fresh DB (`docker compose down -v && up -d && npm run db:setup` → 2 migrations applied + 1 seed). Same for `db:migrate`'s `[✓]` — it appears for both fresh apply and fast no-op (check the logged `hash/sessions` to distinguish) |
...
| 429 “Search is running too frequently” | The server-side search is rate-limited (10 / 10 s per session) as defense-in-depth for very short terms that cannot use the trigram index; the dialog falls back to title filtering — wait a moment and retry |
| 429 “Too many new sessions from this network” | New-session creation is bounded to 60 / 10 s per network (valid-cookie requests are never throttled); a scripted client, a shared proxy, or a cold-start test run hit the bound — wait a moment and reload |
