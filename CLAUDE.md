...
- `ApiError(status, message)` + `errorResponse()` is the single error funnel for API routes; user-facing copy is specific ("what happened, what to do"). Every response — success or error — carries `Cache-Control: no-store` (pinned by the malformed-input API test).
