...
- `src/app/api/conversations/` — list / search / read / rename / delete. The list endpoint accepts `?q=` (server-side ILIKE over title and JSONB message content, owner-filtered, wildcard-escaped); the `?q=` path is rate-limited per session (10 searches / 10 s, process-local `src/lib/throttle.ts`) because the JSONB expansion costs real DB CPU on image-heavy workspaces — plain listing stays unlimited. Every query filters by `owner`; delete uses a transaction with `FOR UPDATE` on the session row so it cannot race a running generation.
...
- `src/lib/server.ts` — cookie session (`kimi_session`, 64-hex token; only the SHA-256 digest is stored as `owner`), `assertOrigin` (same-origin writes), bounded `readJson` (3 MB), `errorResponse` (structured logs without PII; every response — success or error — carries `Cache-Control: no-store`).
...
- Limits are enforced server-side (60 messages/conversation, 100 conversations/workspace, 16k chars, 2 MB image — with a 2.8M-char base64 data-URL pre-cap in `imageSchema` that stays ahead of the 2 MB binary decode check — 1,200k response chars up to 256k tokens; ~16 MB history; plus 10 searches / 10 s / session on the `?q=` path); the client mirrors some of them but the server is the authority.
...
- Operational debt: a private key was once committed at `docs/ssh-key.txt` (removed from tracking; still in git history — see `docs/CODE_REVIEW_REPORT.md`) and a provider key was once committed in `.env` (untracked in pass 3). Treat BOTH rotations as pending until the operator confirms them; never reintroduce key material or track `.env`. Status check 2026-09-12 (pass 9): the live deployment's `NVIDIA_API_KEY` is present (`configured: true`) and the failure signature CHANGED — instead of the fast 401/403 key-rejection banner, the provider fetch now stalls with no events for >200 s after `meta` and the browser sees the network-interrupted copy (Cloudflare cuts the idle connection at ~100 s; keep-alive frames now keep the wire alive, but the round-trip itself still needs operator action: verify/rotate the key, check NVIDIA NIM status for `moonshotai/kimi-k3`, verify server egress — see `docs/CODE_REVIEW_REPORT.md` pass 9 A2).
