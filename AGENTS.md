...
- `src/lib/keepalive.ts` — pure `startKeepAlive(beat, intervalMs)`: interval math only (RangeError outside 1–600,000 safe integers), returns a stop function (idempotent). The route wires it to enqueue `: keep-alive` comment frames guarded by the abort signal; unit-tested in `tests/heartbeat.test.mjs` together with the parser's comment-frame tolerance.
