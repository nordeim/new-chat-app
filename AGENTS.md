...
- `src/app/api/chat/route.ts` — the core: origin check → session → zod validation → image magic-byte check → atomic session lease (one generation per workspace, 3 s spacing, 195 s expiry; 615 s for `max_tokens > 16,384`) → provider-key check (503 when `NVIDIA_API_KEY` is unset — deliberately AFTER the lease so the lease is the uniform admission gate: conflict behavior (429) is reachable and tested without a provider key; the key-check throw releases the lease, and an immediate retry inside the 3 s spacing window sees the 429 copy) → conversation upsert with duplicate-retry guard → NVIDIA fetch with SSE parse (timeout 175 s; 590 s for large outputs, `maxDuration 600`) → persistence of final answer only → SSE to browser. While the provider is silent the route emits `: keep-alive` SSE comment frames every 15 s (`src/lib/keepalive.ts`) so proxies (Cloudflare ~100 s, nginx `proxy_read_timeout`) do not tear down the idle connection before the route's own timeout can deliver its curated error.
- `src/app/api/conversations/` — list / search / read / rename / delete. The list endpoint accepts `?q=` (server-side ILIKE over the database-maintained `search_text` column — title + message contents, owner-filtered, wildcard-escaped, term whitespace collapsed — served by the `pg_trgm` GIN index `conversations_search_idx`, so search no longer expands JSONB at query time); the `?q=` path is rate-limited per session (10 searches / 10 s, process-local `src/lib/throttle.ts`) as defense-in-depth — terms under three characters cannot use the trigram index and would degrade to sequential scans. Every query filters by `owner`; delete uses a transaction with `FOR UPDATE` on the session row so it cannot race a running generation.
...
- `src/lib/server.ts` — cookie session (`kimi_session`, 64-hex token; only the SHA-256 digest is stored as `owner`), per-network session-mint throttle (60 new sessions / 10 s; key derived by `src/lib/client-key.ts`: `cf-connecting-ip` → rightmost `x-forwarded-for` value → `"direct"`, IP-shape validated so spoofed first values cannot mint buckets; requests with a valid cookie are never throttled), `assertOrigin` (same-origin writes), bounded `readJson` (3 MB), `errorResponse` (structured logs without PII; every response — success or error — carries `Cache-Control: no-store`).
...
- `src/lib/sse.ts` — SSE parser handling LF/CRLF/CR separators, events split across network chunks, and multi-line data with incremental size limits; the per-event bound is a validated constructor argument (RangeError outside 1–8M safe integers): the provider path keeps the 1M default, the browser constructs 8M because the final `done` event can carry a ~1.2M-char answer JSON-escaped into one data line. Used by BOTH the API route and the browser client. Changes affect both sides. Non-`data:` lines (comments such as the route's `: keep-alive` frames) are ignored by design — never repurpose comment frames as events.
- `src/lib/keepalive.ts` — pure `startKeepAlive(beat, intervalMs)`: interval math only (RangeError outside 1–600,000 safe integers), returns a stop function (idempotent). The route wires it to enqueue `: keep-alive` comment frames guarded by the abort signal and a try/catch (an errored controller throws on enqueue — expected teardown, never a crash); unit-tested in `tests/heartbeat.test.mjs` together with the parser's comment-frame tolerance.
- `src/lib/throttle.ts` — pure `createRateLimiter({windowMs, max, maxEntries?, clock?})` sliding-window limiter (RangeError on invalid config, injectable clock, entry-capped key map so unique-key floods cannot grow the entry count). Two module-level call sites: the conversations route's `?q=` path and the session-mint path in `src/lib/server.ts`; unit-tested in `tests/throttle.test.mjs` (the mint key derivation has its own suite, `tests/client-key.test.mjs`).
...
- `src/db/schema.ts` — Drizzle schema. `messages` is JSONB on `conversations`; `search_text` is a STORED generated column (`title || E'\n' || messages_content_text(messages)`, IMMUTABLE helper from migration 0001 that extracts message contents and excludes image payloads) served by the `conversations_search_idx` GIN `gin_trgm_ops` index; cascade delete via session FK. The helper also lives in `infrastructure/postgres/init/00-create-extensions.sql` so `npx drizzle-kit push` works on a cold database.
...
- Limits are enforced server-side (60 messages/conversation, 100 conversations/workspace, 16k chars, 2 MB image — with a 2.8M-char base64 data-URL pre-cap in `imageSchema` that stays ahead of the 2 MB binary decode check — 1,200k response chars up to 256k tokens; ~16 MB history; plus 10 searches / 10 s / session on the `?q=` path and 60 new sessions / 10 s / network on session minting); the client mirrors some of them but the server is the authority.
...
- Operational debt: a private key was once committed at `docs/ssh-key.txt` (removed from tracking; still in git history — see `docs/CODE_REVIEW_REPORT.md`) and a provider key was once committed in `.env` (untracked in pass 3). Treat BOTH rotations as pending until the operator confirms them; never reintroduce key material or track `.env`. Status check 2026-09-13 (session 5): the pass-9 provider stall (A2) is resolved live — the operator redeployed with a valid key, and the full live suite is 12/12 including a verified provider round-trip (streamed, persisted, reasoning stripped, self-cleaning) plus a clean exploratory pass over rename/export/search/image/delete with zero console errors. The historical-key rotation items (SSH key + the old `nvapi-` key in git history) remain operator actions; they are a hygiene debt, not a live outage.
- Deployments behind TLS ingress must preserve the public `Host` or forward it as `x-forwarded-host` (Cloudflare/nginx do); otherwise the same-origin gate rejects browser writes with 403. README troubleshooting documents the failure mode. The CSP baseline allows the Cloudflare Web Analytics origins (`script-src https://static.cloudflareinsights.com`, `connect-src https://cloudflareinsights.com`) because zones with Web Analytics enabled inject that beacon zone-wide — a verified live failure mode (blocked beacon → console error on every page load); operators not using it can disable Web Analytics in the Cloudflare dashboard.
