...
test("network failure while opening a conversation shows curated copy, not a raw browser error", async ({ page }) => {
  await page.route("**/api/conversations", (route) => route.fulfill({ json: { conversations: [alpha, beta], configured: false } }));
  await page.route(`**/api/conversations/${alpha.id}`, (route) => route.abort("connectionreset"));
  await page.goto("/");
  await page.getByRole("button", { name: "Alpha planning" }).click();
