...
- `src/components/chat-workspace.tsx` — the client workspace shell (history, composer, dialogs). Client-side zod schemas validate every API and stream payload. Markdown rendering lives in `markdown-message.tsx`; image previews in `image-lightbox.tsx`.
- `src/lib/history.ts` — period grouping and relative timestamps for the sidebar.
